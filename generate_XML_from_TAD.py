...
def convert_pred_to_TAS_format(truth, preds, gold_xml_file=None):
    ...
    for pred in preds:
        ...
        for p in pred:
            if phr_sen == '':
                ...
                match = re.match(
                    r"((.*(\s\s)((food|drinks|service|ambience|location|restaurant)(\s)+(general|prices|quality|style_options|miscellaneous))(\s)*)+)",
                    p)
            if match:
    ...
    for pred_offset in range(3):
        ...
        print(f"\n\n\n*******\n{eval_task}{run}_{dir_prefix}_sentence{pred_offset}.xml'\n*********\n\n")
